'''
Для тестирования масштабных изменений.
На данный момент представлена копия модуля chord_position
 '''
...
def position_raw():
    ...
    for k, i in enumerate(st.string_list()):  # для каждой струны
        ...
        if pos_raw[k] == []:  # если не нашлось нужной ноты
            pos_raw[k] = [-1]  # -1 - маркер того, что струна не играется
    return pos_raw
        ...
        for l, j in enumerate(i):
            if l <= (len(st.string_values)-len(chord_simple_value(note,chord))) and not stop_check :
                ...
                if j%12 != chord_simple_value(note,chord)[0]:
                    ...
                    stop_check = True  #перестаем проверять на тонику
            else:
                pure_chord[k].append(j)
...
def test_all():
    '''Прогоняет через все возможные комбинации. Среднее время 2m.'''
    for i in nv.allnotes[:1]:
        for k,v in types.dict.items():
...
chord = 'major' #список аккордов в chord_types
fret_min = 0
fret_max = 3
